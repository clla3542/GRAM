...
Quality Check:
    select good, unsure, or bad quality. If 'bad' is selected, next target will appear
Location:
    bottom left image is from DSS database, roboao field is marked in faint brown box. Smaller green-outlined image 
    ...
    previous position. Can also tag image for not enough stars or bad pointing.
Companion:
    select 'no companion' or choose location of companion by clicking on the companion in the left-most of the four
    bottom images (each 1.5 arcsec across). For distant compaion, click in upper right image (8 arcsec across). 
    Position is overwritten each time new position is selected.
...
nlist = [s for s in os.listdir(path) if s.startswith('3_')]                     #make list of directory path  
...
class MyApp(object):
    ...
    def __init__(self, root):
        ...
        frame3.grid(row=1, column=7, rowspan=7, columnspan=1, sticky=S)    #location on grid, y space, stuck to West
        ...
        def nxtTarget(f):   #Function to bring up individual information for each target
            ...
            if os.path.isfile(dpath[ct]+'/guiOut.txt')==False:                  #only create file if one doesn't already exist
                ...
                guiOut.write("%s \n%s \n%s \n%s \n%s \n%s \n%s \n%s \n%s \n%s \n%s \n" % ('Target:'+names[ct],       #create blank template    
                    'G:x', 'U:-', 'B:-', 'NES:-', 'IP:-', 'CB:-', 'NM:-', 'Location:-', 'Companion:-', 'Far Companion:-'))
            ...
            Label(root, text=names[ct], fg='white', bg='black', font=("Helvetica", 16))\
                  .grid(row=0, column=0, columnspan=4, sticky=W)    #target name (color, font, location)
            ...
            .grid(row=0, column=7, sticky=E)                        #status on total targets, current/total (color, font, location)
            ...
            label5.image = photo5
            label2.grid(row=2, column=0, rowspan=4, sticky=N)                     #contrast curve location
        ...
        def selectloc1(event):                                                     
            ...
            editOut(8, 'Location: ('+str(fx)+' '+str(fy)+')')
        ...
        def selectloc2(event):
            ...
            editOut(9, 'Companion: ('+str(fx)+' '+str(fy)+')')                  #update out text file
        ...
        def selectloc3(event):
            ...
            editOut(10, 'Far Companion: ('+str(fx)+' '+str(fy)+')')             #update out text file
        ...
        #Position Check
        canvas1 = Canvas(frame1, width=150, height=150, highlightthickness=0, cursor='@arrow.cur')  #create canvas, size, cursor
        ...
        circle4 = canvas2.create_oval(500, 500, 501, 501, outline='chartreuse2')
        canvas2.bind("<ButtonPress-1>", selectloc2)                                     #if click here, call selectloc2 function
        
        canvas3 = Canvas(frame3, width=150, height=150, highlightthickness=0, cursor='@arrow.cur')  #canvas frame, location, cursor
        canvas3.pack(expand = YES, fill = BOTH)
        img3 = canvas3.create_image(0, 0, anchor=NW)                                    #set up for but don't include image yet
        circle5 = canvas3.create_oval(500, 500, 501, 501, outline='chartreuse2')        #create out-of-sight circle
        ...
        Label(root, text='COMPANION', fg='cadet blue', bg='black', font=("Helvetica", 12, 'bold'))\
            .grid(row=2, column=6, padx=20, sticky=N)
        #Image Labels
        ...
        Label(root, text='Full Field Fits', fg='DarkOliveGreen4', bg='black', font=('Helvetica', 8))\
        ...
        Label(root, text='Field = 8" across', fg='cadet blue', bg='black', font=('Helvetica', 8))\
            .grid(row=8, column=7, sticky=NW, padx=10)                        #Full field
        ...
        LOk = Button(root, text = 'Reset Location', command=lambda: sequence(editOut(8, 'Location:-'), rmCircle(0), editOut(5, 'INP:-'), editOut(4, 'NES:-')),\
              height=1, width=15, bg='DarkOliveGreen4', font=('Helvetica', 12)).grid(row=5, column=4, sticky=N, padx=20)  #location is OK
        NC = Button(root, text="Nothing", command=lambda: sequence(editOut(9, 'Companion:-'), editOut(10, 'Far Companion:-'), rmCircle(1), editOut(6, 'CB:-'), editOut(7, 'NM:-')),\
              height=1, width=15, bg='cadet blue', font=('Helvetica', 12)).grid(row=5, column=6, padx=20, sticky=NE)  #no companion
        CB = Button(root, text="Poss. Close Binary", command=lambda: editOut(6, 'CB:x'),\
              height=1, width=15, bg='cadet blue', font=('Helvetica', 12)).grid(row=3, column=6, padx=20, sticky=NE)  #possible close binary
        NM = Button(root, text="Look at Manually", command=lambda: editOut(7, 'NM:x'),\
              height=1, width=15, bg='cadet blue', font=('Helvetica', 12)).grid(row=4, column=6, padx=20, sticky=NE)  #no companion
